...
_DEFAULT_CLF_PARAMS = {"C": 1.0, "max_iter": 1000, "random_state": 42}
_DEFAULT_REG_PARAMS = {"alpha": 1.0}
...
class LinearRegressionModel(BaseModel):
    def train(self, X: np.ndarray, y: np.ndarray, **kwargs) -> Dict[str, Any]:
        ...
        logger.info("Linear model (%s) trained — metrics: %s", self.config.task.value, self.metrics)
        return self.metrics
