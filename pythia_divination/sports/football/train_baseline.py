"""Train a first Football home-win baseline model."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a Football baseline home-win model.")
    parser.add_argument("--dataset", default=None, help="Optional explicit dataset CSV path.")
    ...
    parser.add_argument(
        "--output-dir",
        default=str(Path(__file__).resolve().parents[2] / "artifacts" / "football_baseline"),
...
def _time_split(dataset: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if unique_count < 64:
        raise ValueError("Need at least 64 completed Football games to train a baseline split.")
...
def train_baseline(args: argparse.Namespace) -> dict[str, Any]:
    ...
    output_dir = Path(args.output_dir) / "nfl" / args.model / "home_win"
    ...
    logger.info("Football baseline metrics: %s", json.dumps(metrics, indent=2))
    return metrics
