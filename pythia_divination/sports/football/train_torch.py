"""Train a PyTorch classifier for Football home-win prediction."""
...
from sports.football.train_baseline import _IDENTIFIER_COLUMNS, _LABEL_COLUMNS
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a Football PyTorch home-win model.")
    parser.add_argument("--dataset", default=None, help="Optional explicit Football training dataset path.")
    parser.add_argument("--rebuild-dataset", action="store_true", help="Rebuild the dataset before training.")
    parser.add_argument("--season-start", type=int, default=2020, help="First season to include if rebuilding.")
    ...
    parser.add_argument(
        "--output-dir",
        default=str(ROOT / "artifacts" / "football_torch"),
...
def _default_dataset_path() -> Path:
    return DEFAULT_FOOTBALL_DATA_ROOT / "normalized" / "football_training_dataset_latest.csv"
...
def _time_split(dataset: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if unique_count < 64:
        raise ValueError("Need at least 64 games to train the Football torch model.")
...
class FootballDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> FootballBatch:
        return FootballBatch(self.x[index], self.y[index])


def _collate(batch: list[FootballBatch]) -> FootballBatch:
    return FootballBatch(
...
def _predict(model: FootballTorchModel, loader: DataLoader, *, device: torch.device) -> np.ndarray:
    model.eval()
...
def train_torch(args: argparse.Namespace) -> dict[str, Any]:
    ...
    train_loader = DataLoader(
        FootballDataset(x_train, y_train),
    ...
    model = FootballTorchModel(input_dim=len(feature_columns), hidden_width=args.hidden_width, dropout=args.dropout).to(device)
    ...
    output_dir = Path(args.output_dir) / "nfl" / "home_win"
    ...
    pd.DataFrame(
        {
            ...
            "away_team": val_df["away_team"].values,
            "home_team": val_df["home_team"].values,
            "home_win": y_val.astype(int),
            "home_win_probability": probabilities,
        }
    ).to_csv(output_dir / "validation_predictions.csv", index=False)
    pd.DataFrame(history).to_csv(output_dir / "training_history.csv", index=False)
    write_json(output_dir / "metrics.json", metrics)
    logger.info("Football torch metrics: %s", json.dumps(metrics, indent=2))
    return metrics
