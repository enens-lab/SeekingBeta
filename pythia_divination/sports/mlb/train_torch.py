"""Train a tabular PyTorch classifier for MLB home-win prediction."""
...
from sports.mlb.build_training_dataset import DEFAULT_MLB_DATA_ROOT, build_training_dataset  # noqa: E402
from sports.mlb.train_baseline import _LABEL_COLUMNS, _IDENTIFIER_COLUMNS  # noqa: E402
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train an MLB PyTorch home-win model.")
    parser.add_argument(
        "--dataset",
        default=str(DEFAULT_MLB_DATA_ROOT / "normalized" / "mlb_training_dataset_latest.csv"),
        help="Path to MLB training dataset.",
    )
    ...
    parser.add_argument("--season-start", type=int, default=None, help="First season to build if rebuilding.")
    parser.add_argument("--season-end", type=int, default=None, help="Last season to build if rebuilding.")
    parser.add_argument("--disable-statcast-cache", action="store_true", help="Disable auto-loading cached Statcast daily files.")
    parser.add_argument("--statcast-path", action="append", default=[], help="Optional explicit Statcast file path.")
    ...
    parser.add_argument("--label-smoothing", type=float, default=0.0, help="Optional binary label smoothing.")
    ...
    parser.add_argument(
        "--output-dir",
        default=str(ROOT / "artifacts" / "mlb_torch" / "home_win"),
...
def _time_split(dataset: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if unique_count < 20:
        raise ValueError("Need at least 20 games to train the MLB torch model.")
...
class MLBDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> MLBBatch:
        return MLBBatch(self.x[index], self.y[index])


def _collate(batch: list[MLBBatch]) -> MLBBatch:
    return MLBBatch(
...
class MLBTorchModel(nn.Module):
    def __init__(self, input_dim: int, hidden_width: int, dropout: float) -> None:
        ...
        self.net = nn.Sequential(
            ...
            nn.Dropout(dropout),
            nn.Linear(hidden_width, max(128, hidden_width // 2)),
            nn.ReLU(),
            nn.BatchNorm1d(max(128, hidden_width // 2)),
            nn.Dropout(dropout),
            nn.Linear(max(128, hidden_width // 2), max(64, hidden_width // 4)),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(max(64, hidden_width // 4), 1),
        )
...
def _predict(model: MLBTorchModel, loader: DataLoader, *, device: torch.device) -> np.ndarray:
    model.eval()
...
def train_torch(args: argparse.Namespace) -> dict[str, Any]:
    ...
    model = MLBTorchModel(input_dim=len(feature_columns), hidden_width=args.hidden_width, dropout=args.dropout).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        auc = roc_auc_score(y_val, probabilities) if len(np.unique(y_val)) > 1 else None
    ...
    val_predictions = val_df[["game_pk", "official_date", "away_team_name", "home_team_name", "home_win"]].copy()
...
def main() -> None:
    ...
    print("=" * 72)
    print("MLB TORCH TRAINING COMPLETE")
    print("=" * 72)
