...
function buildTopDriversSummary(probUp: number | null, selectedHorizon: string): string {
  ...
  if (probUp >= 0.55) {
    return `Upside probability ${probability} over ${selectedHorizon}; trend/momentum inputs are supportive versus recent volatility.`;
  }
  if (probUp <= 0.45) {
    return `Upside probability ${probability} over ${selectedHorizon}; momentum is weaker with elevated downside pressure.`;
  }
  return `Upside probability ${probability} over ${selectedHorizon}; trend, momentum, and volatility inputs are mixed.`;
}
